...
type DataEntitiesCodingAdvance=
...
open WX.Data.BusinessBase") |>ignore
    for a in tableRelatedInfos do
      ...
      | HasNone ->
          DataEntitiesCodingAdvance.GetCodeWithTemplate a.TableName
          |>string |>sb.Append |>ignore
      | _ ->
          DataEntitiesCodingAdvance.GetCodeWithTemplateAndConditions a.TableName
          |>string |>sb.Append |>ignore
      *)
      ...
      | _ ->
          ...
          | ChildTable ->
              DataEntitiesCodingAdvance.GetChildRelationshipsCode a.LevelOneChildTableName
              |>string|>sb.Append|>ignore
          | LeafTable
          | IndependentTable  -> ()
          | _ ->()
...
//    and set v= x._TrackingState <-v")|>ignore
  ...
  static member private GetChildRelationshipsCode (childTableName:string)=
    ...
    sb.AppendFormat(@"
  [<DV>]{0}
  val mutable private _BD_{1}s:List<BD_{1}>
  [<DataMember>]
  member x.BD_{1}s
    with get()=
    ...
    and set v= x._{1} <-v",
              ...
              *)
              ...
              | x,y,true when x.EndsWith("byte[]") || x.EndsWith("string") -> 
                  Environment.NewLine+blankSapce+"[<StringLengthValidator("+y.ToString()+")>]"
                  (*
                  //部分正确的备份
    ...
    and set v=
      if  x._{1}<>v  then
        ...
        x.OnPropertyChanged ""{1}""",
              ...
              | false, EndsWithIn [StringTypeName] _, NotEndsWithIn [JMColumnSuffix] y ->
              ...
              *)
              ...
              | false,x,y when x.EndsWith("string") &&  y=DJHColumnName  -> Environment.NewLine+blankSapce+"[<IgnoreNulls>]"
              ...
              *)
              ...
              //{4}
              match a.DATA_TYPE.ToLowerInvariant(),a.CHARACTER_MAXIMUM_LENGTH,a.IS_NULLABLE_TYPED with 
              | x,y,false when x.EndsWith("string")  -> Environment.NewLine+blankSapce+"[<StringLengthValidator(1,"+y.ToString()+ @",MessageTemplate= @""The length of "+(getColumnDescription a.TABLE_NAME a.COLUMN_NAME)+ @" must fall within the range """"{3}"""" ({4}) - """"{5}"""" ({6}){2}"")>]" 
              | x,y,false when x.EndsWith("byte[]")-> Environment.NewLine+blankSapce+"[<StringLengthValidator(1,"+y.ToString()+ @",MessageTemplate= @""The length of "+(getColumnDescription a.TABLE_NAME a.COLUMN_NAME)+ @" must fall within the range """"{3}"""" ({4}) - """"{5}"""" ({6}){2}"")>]" //包括 Byte []
              | _ -> String.Empty
              ,
              //{5}
              match a.DATA_TYPE.ToLowerInvariant(),a.CHARACTER_MAXIMUM_LENGTH,a.IS_NULLABLE_TYPED with
              | x,y,true when x.EndsWith("byte[]") || x.EndsWith("string") -> 
                  Environment.NewLine+blankSapce+"[<StringLengthValidator("+y.ToString()+ @",MessageTemplate= @""The length of "+(getColumnDescription a.TABLE_NAME a.COLUMN_NAME)+ @" must fall within the range """"{3}"""" ({4}) - """"{5}"""" ({6}){2}"")>]"
                  (*
                  //部分正确的备份
                  Environment.NewLine+blankSapce+ @"[<ValidatorComposition(CompositionType.Or,MessageTemplate=""The string characters can not more than "+y.ToString()+  @" !"")>]"+  //包括 Byte []
                  Environment.NewLine+blankSapce+"[<StringLengthValidator("+y.ToString()+")>]"+  //包括 Byte []
              ...
              *)
              ...
              | x,  _ when x.EndsWith("int64") -> Environment.NewLine+blankSapce+"[<RangeValidator("+string Int64.MinValue+",RangeBoundaryType.Exclusive,"+string Int64.MaxValue+ @",RangeBoundaryType.Exclusive,MessageTemplate= @""The value of "+(getColumnDescription a.TABLE_NAME a.COLUMN_NAME)+ @" must fall within the range """"{3}"""" ({4}) - """"{5}"""" ({6}){2}"")>]"
              | x,  _ when x.EndsWith("double") -> Environment.NewLine+blankSapce+"[<RangeValidator("+string Double.MinValue+",RangeBoundaryType.Exclusive,"+string Double.MaxValue+ @",RangeBoundaryType.Exclusive,MessageTemplate= @""The value of "+(getColumnDescription a.TABLE_NAME a.COLUMN_NAME)+ @" must fall within the range """"{3}"""" ({4}) - """"{5}"""" ({6}){2}"")>]"
              | x, (y,z) when x.EndsWith("decimal") -> 
                    let maxValueString=
                      match 10.0**float y-10.0**float -z with
                      | zx when z>0 ->zx |>string
