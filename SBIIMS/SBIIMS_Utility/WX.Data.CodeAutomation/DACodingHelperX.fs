...
type DACodingHelperX=
  static member GetCode (databaseInstanceName:string) (tableRelatedInfos:TableRelatedInfoX[])=  
    let sb=StringBuilder()
    try
      sb.Clear()|>ignore
      DACodingHelperX.GenerateNamespaceCode
      |>string|>sb.Append|>ignore
      sb.AppendLine()|>ignore
      DACodingHelperX.GenerateDAHelperCode databaseInstanceName tableRelatedInfos    //获取单据名名称
      |>string|>sb.Append|>ignore
      sb.AppendLine()|>ignore
      string sb
    with 
...
open WX.Data.ServerHelper"
  ...
  //该类中仅处理共享Helper的方法
  static member private GenerateDAHelperCode (databaseInstanceName:string)  (tableRelatedInfos:TableRelatedInfoX [])= 
    let sb=StringBuilder()
    sb.AppendFormat(@"
type  DA_{0}Helper=()",databaseInstanceName)|>ignore
    string sb
    (*已移植到DACodingBusinessHelperX中
    match tableRelatedInfos|>Array.exists (fun a->a.TableInfo.TABLE_NAME="T_DJLX") with
    | true  ->
        DACodingHelperX.GenerateDAHelperGetDJHCode databaseInstanceName //单据号
        |>string |>sb.Append |>ignore
        sb.AppendLine()|>ignore
        DACodingHelperX.GenerateDAHelperGetLSHCode databaseInstanceName //流水号
        |>string |>sb.Append |>ignore
        sb.AppendLine()|>ignore
        DACodingHelperX.GenerateDAHelperGetDJH2MCode(databaseInstanceName) //单据号2M
        |>string |>sb.Append |>ignore
        sb.AppendLine()|>ignore
        (*已停用
        DACodingHelperX.GenerateDAHelperUpdateLSHCode databaseInstanceName //更新流水号
        |>string |>sb.Append |>ignore
        *)
        DACodingHelperX.GenerateDAHelperDJNameCode(databaseInstanceName)
        |>string |>sb.Append |>ignore
        sb.AppendLine()|>ignore
    | _ ->()
    DACodingHelperX.GenerateDAHelperTrackingInfoCode()  //BusinessEntity公共的跟踪信息
    |>string |>sb.Append |>ignore
    sb.AppendLine()|>ignore
    DACodingHelperX.GenerateDAHelperWriteBusinessLogCode databaseInstanceName //写业务操作日志
    |>string |>sb.Append |>ignore
    sb.AppendLine()|>ignore
    string sb

  static member private GenerateDAHelperDJNameCode(databaseInstanceName)=
  ...
  static member GetDJName (code:byte)=
    ...
    {0}",
        ...
        for a in DatabaseInformationX.GetDJLX(databaseInstanceName) do
          sbTem.AppendFormat(@"
    | {0}uy->""{1}""",
            //{0}
            a.C_DM
  ...
  static member private GenerateDAHelperGetDJH2MCode(databaseInstanceName)=
    let sbTem=StringBuilder()
    let sb=StringBuilder()
    try
      sb.AppendFormat(  @"
