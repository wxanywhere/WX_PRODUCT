...
type WcfServiceCodingAdvance=
  static member GetCode (databaseInstanceName:string) (tableRelatedInfos:TableRelatedInfo seq)=  //static member GetCode (typedTableNames:(string*TableTemplateType) list)=
    ...
    sb.AppendFormat( @"namespace WX.Data.WcfService
open System
  ...
  interface  IWS_{0} with"
    ...
    for a in tableRelatedInfos do
      ...
      | MainTableWithTwoLevels->
          WcfServiceCodingAdvance.GetCodeWithTemplateOne a.TableName
          |>string |>sb.Append |>ignore
          sb.AppendLine()|>ignore
      | IndependentTable ->
          match a.ColumnConditionTypes with
          | ColumnConditionTypeContains [HasXH] _ ->
              WcfServiceCodingAdvance.GetCodeWithTemplateThree a.TableName
              |>string |>sb.Append |>ignore
              sb.AppendLine()|>ignore
          | _ ->
              WcfServiceCodingAdvance.GetCodeWithTemplateOne a.TableName
      ...
      | ChildTable 
      | LeafTable -> 
          WcfServiceCodingAdvance.GetCodeWithTemplateTwo a.TableName
          |>string |>sb.Append |>ignore
          sb.AppendLine()|>ignore
      | _ ->()
    string sb
